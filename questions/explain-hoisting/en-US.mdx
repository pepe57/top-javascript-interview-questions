...
<KnowledgeCheck id="function-declaration-vs-expression" type="single-select">
  ...
  <KnowledgeCheck.Option value="both-log-values">
    Both strings are logged successfully.
  </KnowledgeCheck.Option>
  <KnowledgeCheck.Option value="declaration-then-type-error" correct>
    `'function declaration'` is logged, then calling `expressed` throws a
    `TypeError`.
  </KnowledgeCheck.Option>
  <KnowledgeCheck.Option value="reference-error-before-output">
    A `ReferenceError` is thrown before anything is logged.
  </KnowledgeCheck.Option>
  <KnowledgeCheck.Explanation>
    Function declarations are initialized with their function body before
    execution. The `var expressed` binding is also available, but its function
    assignment has not run yet, so its value is `undefined` and calling it
    throws a `TypeError`.
  </KnowledgeCheck.Explanation>
</KnowledgeCheck>
